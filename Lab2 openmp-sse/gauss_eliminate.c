...
main(int argc, char** argv) {
	...
	gettimeofday(&stop, NULL);
	printf("CPU run time with openmp = %0.2f s. \n", (float)(stop.tv_sec - start.tv_sec + (stop.tv_usec - start.tv_usec)/(float)1000000));
...
void 
gauss_eliminate_using_openmp(const Matrix Aa, Matrix Uu)                  /* Write code to perform gaussian elimination using OpenMP. */
{
	unsigned int i, j, k;
	
	float* A = Aa.elements;
	float* U = Uu.elements;
	int num_elements = Aa.num_rows;
    ...
    for (k = 0; k < num_elements; k++){             /* Perform Gaussian elimination in place on the U matrix. */
		#pragma omp parallel for private(j) shared(U, num_elements, k)
		...
		#pragma omp parallel for private(i, j) shared(U, num_elements, k) 
