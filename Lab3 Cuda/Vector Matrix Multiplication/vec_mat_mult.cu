...
main(int argc, char** argv) {
	...
	printf("Execution time with reference = %fs. \n", (float)(stop.tv_sec - start.tv_sec + (stop.tv_usec - start.tv_usec)/(float)1000000));
...
vec_mat_mult_on_device_using_global_memory(const Matrix A, const Matrix X, Matrix Y)
{
	...
	dim3 threads(256, 1);
	dim3 grid(MATRIX_SIZE/threads.y, 1);
	...
	gettimeofday(&stop, NULL);
	printf("Execution time with global memory = %fs. \n", (float)(stop.tv_sec - start.tv_sec + (stop.tv_usec - start.tv_usec)/(float)1000000));
...
vec_mat_mult_on_device_using_shared_memory(const Matrix A, const Matrix X, Matrix Y)
{
	...
	threads.z = 1;
	grid.x = (MATRIX_SIZE / threads.x) + ((MATRIX_SIZE % threads.x) ? 1:0 );
	grid.y = (MATRIX_SIZE / threads.y) + ((MATRIX_SIZE % threads.y) ? 1:0 );
	grid.z = 1;
	...
	gettimeofday(&stop, NULL);
	printf("Execution time with shared memory = %fs. \n", (float)(stop.tv_sec - start.tv_sec + (stop.tv_usec - start.tv_usec)/(float)1000000));
